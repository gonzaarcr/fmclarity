...
export default function Table<T>({
  ...
  const sortedElements = sortOrder === undefined ? elements : elements.sort((e1: T, e2: T) => {
            ...
            {columns.map(col => (
              <th key={col.field} className={styles.cell}>
                <a href="#" onClick={() => changeSortOrder(col.field)}>
                  {col.headerText ?? col.field} {sortOrder?.field === col.field && ORDER_ICON[sortOrder.ord]}
                </a>
              </th>
            ))}
        ...
        <tbody>
          {sortedElements.map(e => (
            <tr key={getKey(e)} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
              ...
              <td className={`${styles.cell} ${styles.edit_cell}`}>
                <a href="#" className={styles.edit_text}>Edit</a>
              </td>
            </tr>
          ))}
        </tbody>
